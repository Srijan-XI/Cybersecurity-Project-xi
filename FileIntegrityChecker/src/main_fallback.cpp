#include <iostream>
#include <string>
#include "FileHasherFallback.h"
#include "FileManager.h"
#include "IntegrityChecker.h"

void printUsage() {
    std::cout << "🛡️  File Integrity Checker (Fallback Version)\n"
              << "===============================================\n"
              << "Usage:\n"
              ...
              << "Note: This version uses CRC32 hashing (not cryptographically secure)\n"
              << "For production use, please build with OpenSSL for SHA-256 support.\n\n"
              << "Examples:\n"
...
class FallbackIntegrityChecker {
private:
    FileManager& fileManager;

public:
    FallbackIntegrityChecker(FileManager& fm) : fileManager(fm) {}
    ...
    bool checkIntegrity(const std::string& filepath) {
        ...
        try {
            currentHash = FileHasherFallback::calculateCRC32(filepath);
...
int main(int argc, char* argv[]) {
    ...
    FallbackIntegrityChecker checker(fileManager);

    if (command == "add" && argc == 3) {
        std::string filepath = argv[2];
        try {
            std::string hash = FileHasherFallback::calculateCRC32(filepath);
            if (fileManager.containsFile(filepath)) {
